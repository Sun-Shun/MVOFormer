...
class RAFT(
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None, test_mode=False, only_context=False):
        ...
        img12 = torch.cat([image1, image2], dim=1)
        cnet = self.cnet(img12)
        ...
        for itr in range(iters):
            ...
            coor = coords_grid(N, H, W, device=image1.device)
        ...
        if test_mode == False:
            ...
            return {'final': flow_predictions[-1], 'flow': flow_predictions, 'info': info_predictions[-1], 'nf': nf_predictions, 'context': context}
        else:
            return {'final': flow_predictions[-1], 'flow': flow_predictions, 'info': info_predictions[-1], 'nf': None, 'context': net}